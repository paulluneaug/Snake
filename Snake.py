...
def play_snake():
    ...
    """
    global c,x_can,y_can,speed,fen,can,fruit,snake,dir_changeable

    x_can,y_can=15,15 #Nombre de case en largeur et en hauteur de la grille sur
                      #laquelle le serpent se déplace

    c=700//max(x_can,y_can) #Taille en pixels de chaque case
    speed=200 #Temps en ms entre chaque mise à jour de la grille
    ...
    snake=[[(0,0)],(1,0)] #Liste indiquant l'état du serpent
...
def can_update(update_fruit=False):
    ...
    for p in range (len_snake):
        ...
        can.create_rectangle((pix[0]+pad)*c,(pix[1]+pad)*c,(pix[0]+1-pad)*c,
                             (pix[1]+1-pad)*c,fill='red',tag='snake')
...
def lose():
    ...
    can.create_text(x_can*c/2,y_can*c/2-20,text='Perdu',font='Verdana 20',
                    fill='red')
    can.create_text(x_can*c/2,y_can*c/2+20,font='Verdana 15',fill='red',
